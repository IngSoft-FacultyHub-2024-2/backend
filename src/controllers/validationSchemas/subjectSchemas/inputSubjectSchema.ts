...
const inputSubjectSchema = yup.object().shape({
    ...
    study_plan_id: yup.number().typeError('La materia debe pertenecer a un plan de estudios').integer().positive().required('El plan de estudio es obligatorio.'),
    associated_coordinator: yup.number().typeError('La materia debe tener un coordinador').integer().positive().required('El coordinador asociado es obligatorio.'),
    index: yup.number().typeError('El indice debe ser un numero').positive('El indice debe ser un numero positivo').required('El índice de la materia es obligatorio y debe ser mayor que cero.'),
    ...
    hour_configs:  yup.array().of(inputHourConfigSchema),
