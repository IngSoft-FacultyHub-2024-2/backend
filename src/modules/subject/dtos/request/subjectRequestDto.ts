import {
  HourConfigRequestDto,
  HourConfigRequestDtoHelper,
} from './hourConfigRequestDto';
...
import {
  ...
  SubjectEventRequestDtoHelper,
} from './subjectEventRequestDto';
...
export class SubjectRequestDtoHelper {
  ...
  public static toModel(dto: SubjectRequestDto): Partial<Subject> {
    return {
      ...
      ...(dto.subject_folder !== undefined && {
        subject_folder: dto.subject_folder,
      }),
      ...(dto.technologies !== undefined && { technologies: dto.technologies }),
      ...(dto.notes !== undefined && { notes: dto.notes }),
      is_teo_tec_at_same_time: dto.is_teo_tec_at_same_time,
      ...(dto.valid !== undefined && { valid: dto.valid }),
      hour_configs:
        dto.hour_configs?.map(
          (hc) => HourConfigRequestDtoHelper.toModel(hc) as HourConfig
        ) || ([] as HourConfig[]),
      needs_ids: dto.needs_ids || [],
      needs_notes: dto.needs_notes,
      events:
        dto.events?.map(
          (e) => SubjectEventRequestDtoHelper.toModel(e) as SubjectEvent
        ) || ([] as SubjectEvent[]),
    };
  }
}
